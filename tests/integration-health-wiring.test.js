...
describe('the mark that outlives the card', () => {
  it('is on the navigation for every watched service', () => {
    ...
    expect(indexContent.match(/data-health-badge="shippo,canadapost,shipping-email,stripe-fees"/g)).toHaveLength(2);
  });

  it('starts hidden and is announced to a screen reader', () => {
    expect(indexContent).toMatch(/data-health-badge="shippo,canadapost,shipping-email,stripe-fees"[^>]*aria-label="[^"]+"/);
