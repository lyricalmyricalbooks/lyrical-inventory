  ...
  detailEl.innerHTML = `
    <div class="cf-detail-card">
      ...
      <div class="cf-detail-totals">
        <button class="cf-detail-total ${selectedType === 'income' ? 'active' : ''}" type="button" aria-pressed="${selectedType === 'income' ? 'true' : 'false'}" aria-label="Show income only for ${_tcSvgEsc(label)}" onclick="tcSetCashFlowBucketType('income')"><b>Income</b>${_tcSvgEsc(fmt(total(incomeRows), data.baseCurrency || 'CAD'))}</button>
        <button class="cf-detail-total ${selectedType === 'expense' ? 'active' : ''}" type="button" aria-pressed="${selectedType === 'expense' ? 'true' : 'false'}" aria-label="Show expenses only for ${_tcSvgEsc(label)}" onclick="tcSetCashFlowBucketType('expense')"><b>Expenses</b>${_tcSvgEsc(fmt(total(expenseRows), data.baseCurrency || 'CAD'))}</button>
        <button class="cf-detail-total ${selectedType === 'all' ? 'active' : ''}" type="button" aria-pressed="${selectedType === 'all' ? 'true' : 'false'}" aria-label="Show income and expenses for ${_tcSvgEsc(label)}" onclick="tcSetCashFlowBucketType('all')"><b>Net / All</b>${_tcSvgEsc(fmt((bucket.income || 0) - (bucket.expense || 0), data.baseCurrency || 'CAD'))}</button>
      </div>
