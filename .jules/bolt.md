...
## 2024-05-18 - Pre-computing Map for O(N*M) nested array lookups in shipping analysis
**Learning:** In the POS and shipping analytics dashboard (`buildShippingPnLHtml`), looping over orders (O(N)) and doing `.filter()` over all matching expenses (O(M)) to find postage costs led to an O(N*M) performance bottleneck, especially on large datasets.
**Action:** When performing aggregate metrics combining lists (e.g. Orders and Expenses) matched on an ID, explicitly pre-compute a `Map()` of the secondary list keyed by that ID up front to reduce lookup to O(1) and overall complexity to O(N+M).
